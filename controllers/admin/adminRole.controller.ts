...
const updateMultiAdminRolePatch = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    switch (type) {
      case "status-active":
        await AdminRoleModel.updateMany(
      ...
      case "soft-delete":
        ...
        break;
      case "hard-delete":
        await AdminRoleModel.deleteMany({ _id: { $in: ids } });
        break;
      default:
        break;
    }
...
type TAdminRoleController = {
  ...
  updateMultiAdminRolePatch: (req: Request, res: Response) => Promise<void>;
};
